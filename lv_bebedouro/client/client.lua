...
misc = Tunnel.getInterface("lv_bebedouro")

RegisterNetEvent("lv_bebedouro:use")
AddEventHandler("lv_bebedouro:use", function()
        local ped = GetPlayerPed(-1)
    local pedCoords = GetEntityCoords(ped, 0)

    for k,v in pairs(config.propbebedouro) do 
